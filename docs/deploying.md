...
zone cannot disagree with the repo.
...
**`not_found_handling = "404-page"`** serves a real 404 for anything unknown.
The alternative, `single-page-application`, answers 200 with the landing page for
*every* path, which makes `/desktops/pounce` look like a page. Directory pages
resolve with and without the trailing slash either way (the slashed form is where
the 307 lands), which is why every page carries a canonical tag.
