...
| | |
|---|---|
| **the pages** — `/` the house, `/developers` the machine surface in prose, `/about` `/contact` `/privacy` the house's trust pages, `/perch/privacy` (perch's policy URL, linked from the App Store) | routes under `src/app/`. greyscale at rest, two faces, and one component of our own (`WebMcpTools`), none of it load-bearing. no sheets either — every one is a docs page, with a 301 onto it |
