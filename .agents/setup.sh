...
# container, CI) boot a bare Linux box with no Nix. This repo is NOT a flake —
# but `scripts/sync-nebelung.mjs` shells out to `nix build` to vendor nebelung's
# CSS port, so without Nix you can't refresh the palette block or reproduce what
# the Palette workflow checks (it installs Nix for exactly this reason). This
