...
### Running an item from a link (`urlScheme`)
...
than quietly stripped. Percent-encode a value that contains `&`, `#`, `%` or a
space.
...
*opens* something — a built-in window, a System Settings pane — is not
confirmed: that is what every link on a Mac already does.
