...
```jsonc
{
  ...
  "pages": {
    "enabled": false,        // MRU walk over prefix/* workspaces (needs Accessibility)
    "key": "tab",
    "modifiers": ["ctrl"],
...
`enabled`. The same goes for `appHotkeys` and `pages` below.

### Chords that only fire over one app (`appHotkeys`)

An ordinary hotkey swallows its chord everywhere on the Mac. macOS registers it
globally or not at all, so binding ⌘P costs you printing in every app you own.
`appHotkeys` is the other shape: each `scopes` entry names a `bundleId` and the
chords Pounce should consume **only while that app is frontmost**, passing them
through untouched anywhere else. ⌘P spawns a terminal window over Ghostty and
still prints from your browser.

Each `keys` entry's `target` is an [item key](#per-item-settings-items), the
same address a palette row and `pounce run` use, so a scoped chord and a row
are never two spellings of one command. It needs Accessibility (it rides the
same event tap as `windows`), and a chord held down fires once rather than on
autorepeat.

### Walking workspace pages (`pages`)

Hold the modifier, tap the key: `pages` walks the non-empty AeroSpace
workspaces under `prefix` (`T`, `T/pounce`, `T/haus`) in most-recently-used
order, stepping focus live as you go. There is no HUD, because the screen is
the feedback. Recency comes from `mruFile`, which your window manager's
workspace-change hook writes newest-first; emptiness Pounce already knows from
its window tracker, so a tap never waits on a subprocess. Set `bundleId` to
scope the walk to one app the way a hotkey scopes, which is how a browser keeps
⌃⇥ for its own tabs.

<Callout title="On haus, both of these are written for you">
`haus.terminal.lanes.backend = "zmx"` is the first thing that asks for them: it
generates the ⌘P/⌘⇧P shell-window chords and the ⌃⇥/⌃⇧⇥ walk over its
`T/<repo>` lane pages. See [the Development
room](/docs/haus/rooms/development#ghostty-opens-into-zellij).
</Callout>
