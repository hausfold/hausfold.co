...
`systemSettings` is the one block worth tuning rather than flipping. There are
about 700 individual settings and Accessibility alone ships 342 of them, which is
why two of the three keys exist: `subItemMinQuery` keeps short queries about your
...
files verbatim), which is how a desktop's palette reaches Pounce without a
rebuild. Pounce reads `base`,
...
`bundleIds` lists the row only while one of those apps is frontmost, also
ignoring case; read one off any running app with
...
Either way it scopes the **row**, never a `hotkey` on the same item, the same way
`enabled: false` leaves a key armed. Scoping a chord instead is
...
| `"remap"` | Takes Fn away at the HID layer: it becomes F19, which pounce binds like any ordinary key. No Accessibility, nothing left to race. The cost is that Fn stops being Fn everywhere: no Fn+arrows, no Fn+Delete, no Fn+F1–F12 |
...
Accessibility, and it is armed at startup.
