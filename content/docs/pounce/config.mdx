...
| `stickyTiles` | Tiles hold their positions across summons, so `⌘3` is the same thing next week. Slots go to habit alone and change one at a time, only when a challenger clearly outgrows the weakest; two tiles already on the strip trade places on the same terms, one pair a summon and only when one is worth more than twice the other. Off, the strip is the ranked list's first few rows again and moves whenever the list does |
