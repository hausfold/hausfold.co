...
write into the same header: haus adds its own `cheat` keys for the cheatsheet,
and pounce reads straight past them.
...
The one thing required is *some* whitespace after the `#`. `#pounce: name = x`
is an ordinary comment, and the row it should have named comes out as
`my-command` instead.
</Callout>
