...
When the step the user answers by **picking a row** is followed by another
`pounce`, pass `--chain-rows` instead. A row commit is a separate question from
a typed one, and the same step often wants opposite answers for the same key:
...
has to open into that fade. Land inside it and the new box is taken back down
as the old one finishes closing, which reads as the palette losing a keystroke.
Both flags take an action list, so a picker whose Enter opens a window and whose
