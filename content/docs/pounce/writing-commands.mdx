...
empty match Enter hands the raw text back, and the window holds its loading
skeleton instead of fading between steps:
...
When the step is answered by **picking a row** instead, pass `--chain-rows`:
