...
`arg` is repeatable and positional (`$1`, `$2` …) and only a `cmd:` item takes
any; nothing crosses a shell, so a value with spaces or quotes in it arrives as
one argument. Percent-encode `&`, `#`, `%` or a space.

A link that would **run** something (`cmd:`, `app:`, `shortcut:`) is confirmed
on screen first — the sheet names the app that opened the link and every
argument it carries. A link that only **opens** something (`mode:`, `setting:`)
isn't. Both dials are in [`urlScheme`](/docs/pounce/config#running-an-item-from-a-link-urlscheme):
