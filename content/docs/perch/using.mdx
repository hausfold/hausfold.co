...
| **Your iPhone** | [**Perch Companion**](https://apps.apple.com/app/id6799443735), free on the App Store, once you've [paired it](/docs/perch/install). Its Share sheet sends over your network, or peer-to-peer with no network at all, and the phone can pull shelf items back down too |
...
**Shelf my screenshots** is the same thing with the finding-out done for you: it
opens the folder panel already pointed at wherever this Mac saves captures.
perch cannot simply read that setting, because a sandboxed app is not shown
another app's preferences and cannot grant itself the folder, so the click *is*
the permission. Switching it back off takes that folder out of the watched list
altogether, grant and all.
...
| **Every display** | On by default: each screen gets its own shelf at the notch. Off keeps it to your main display |
| **Launch at login** | perch comes back in the menu bar after a restart: no window, no Dock icon |
| **Devices** | The iPhone listener and the **Pair a Device…** button. Off tears the listener down; pairings are remembered for when it comes back |
