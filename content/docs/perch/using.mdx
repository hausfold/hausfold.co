...
macOS's own. The shelf is a five-second surface, so there is no picker for the
same reason there is none for the accent below: on a haus machine
[`haus.fonts.sans.name`](/docs/haus/rooms/appearance#type) writes it for you,
and moves perch, pounce and trill together. A standalone Mac names it by hand.
