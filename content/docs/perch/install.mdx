...
It never installs the update itself. Instead the button knows how you
installed perch and does the right thing for that install: it copies
`brew upgrade --cask perch`, `haus update` or `nix flake update perch`, or
opens the release page for a drag install. Settings ▸ Updates turns the check
off altogether.
