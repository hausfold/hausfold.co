...
Those five are the whole set. There are no keys waiting to be added, and an
...
frame, the alignment, every row's font and height, the rebuild, and closing the
thing after a click.

```bash
popup_rows() {
  popup_heading --label "Pomodoro" --icon ""
  popup_row --label "Round" --value "3 of 4"
  popup_action --label "Reset" --run "pomodoro reset"
}
```

| kind | for |
|---|---|
| `popup_heading` | a section title. `--count` appends ` · n` when it's above zero |
| `popup_row` | a thing you can act on. `--value` gives it a right-hand column, aligned for you |
