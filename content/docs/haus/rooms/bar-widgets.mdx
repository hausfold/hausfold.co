...
popup_rows() {
  ...
  popup_bar --label "this round" --percentage 62 --value "62%"
...
| kind | for |
|---|---|
| `popup_heading` | a section title, in your widget's hue. A glyph sits in a tinted well. `--count` appends ` · n`; `--badge` puts a capsule on the right |
...
| `popup_button` | a filled, centred control. Tinted by default; `--solid` for the one primary thing to do |
| `popup_bar` | a name, a real track as full as the number, the number |
