...
- **It does not theme or configure the app.** `~/.config/trill/config.json` stays
  yours. trill refuses to move a toggle when that file is a symlink into the Nix
  store, and says so in its own Settings rather than flipping a switch a rebuild
  would revert. Which keys haus should own is a real design question, and this
  room has not answered it.
...
- **[AI](/docs/haus/rooms/ai)**: with this room on, trill's own agent skill is
  installed beside `haus` and holt's, so an agent asked to "tell me when this
...
about the app" means in practice.
