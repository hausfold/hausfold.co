...
`haus.terminal.lanes.backend` is the switch. `"zellij"`, the default, is what the
rows above describe: the lane is a pane in the `main` session. `"zmx"` makes it
its own [zmx](https://github.com/neurosnap/zmx) session in its own Ghostty
window, tiled by windows. The session outlives every client attached to it, so
...
has none by design, so windows tiles real windows rather than a second window
manager nested inside a terminal.

**The rows above change meaning when a lane stops being a pane**, because a
chord only zellij could hear no longer reaches the thing it names. *That backend*
needs `haus.windows.enable`, asserted rather than assumed, since the windows room
is what places a lane and what holds the chord that starts one.

| Keys | Under `"zmx"` |
|---|---|
| `⌃⌘A` | Starts a lane, from anywhere. It replaces `⌘A`/`⌘⇧A`, which are unbound; [the AI room](/docs/haus/rooms/ai) has why the chord had to leave the multiplexer |
| `⌘P` / `⌘⇧P` | Still a new shell in the focused window's directory, but a **window** rather than a pane. `⇧` still stays inside an agent worktree |
| `⌃⇥` / `⌃⇧⇥` | Walk the lane pages by recency, most-recent-first, like `⌘⇥` over apps |
| `⇪ T` | Not one of the rows above but the windows leader's terminal letter, which becomes page-aware: back to whichever lane page you used last |
...
own tabs. The [Launcher](/docs/haus/rooms/launcher) room does that half, which
is why an older pounce simply ignores both chords, and it gains a **Lanes**
command to go with them: fuzzy over every lane with its state, `↵` focusing that
lane's window or waking a parked one, and `/` plus a word searching what the
agents are *saying* across every live session.
