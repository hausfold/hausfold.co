...
panes and tabs inherit your working directory. Ghostty's own `⌘T`, `⌘N`, `⌘Y`,
...
- **`⌘D` / `⌘⇧D` do nothing**, on purpose. Their Ghostty defaults nest a second
  ...
  its layout and pane keys would apply to half the window. Use `⌘N`.
...
is why `⌘⏎` is held above the multiplexer rather than inside it — [the AI
...
- **Agent-worktree aware.** A new shell opened inside an agent worktree hops to
  ...
  throwaway checkout. `⌘⇧N` is the no-hop escape hatch.
