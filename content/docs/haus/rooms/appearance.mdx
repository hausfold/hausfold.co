...
They also land you on a stock Mac rather than on your own old values.
[`haus capture`](/docs/haus/keeping-it-current) before the rebuild is what
records those, and `haus revert-settings` puts them back. It covers the Dock by
default and the contrast key if you name `com.apple.universalaccess`, but not
the display's mode, which lives in no preference domain at all.
</Callout>
