...
front of `pam_tid`, so `sudo` inside the `zmx` session every haus terminal runs
in prompts instead of beachballing. `sudo -v` is the check.
...
`guestAccount` and every `haus.lock.login.*` key writes to
