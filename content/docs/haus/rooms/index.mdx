...
{/* rooms:start — rendered from worker-config.js's ROOMS by `npm run rooms`.
...
{/* rooms:published:start — rendered from worker-config.js's ROOMS by `npm run rooms`.
    Edit the row, not this block: `npm run rooms:check` reverts a hand edit. */}

Nobody else's room is listed here yet. hausfold reads one before listing it,
which is still not vouching for it, and `haus add --room` takes any flakeref in
the meantime.

{/* rooms:published:end */}

[Share a room](/docs/haus/rooms/sharing) is how you publish one of yours.
