...
```nix
# in the receiving room's options.nix
haus._contrib.sill.kettle = contrib.mkExtensionPoint {
...
`config.haus.kettle.something` from inside sill) makes every renamed kettle
