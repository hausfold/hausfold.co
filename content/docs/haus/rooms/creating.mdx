...
Read the path as an address: `bar` is who receives, `kettle` is who is asking.
...
those, `haus.bar` and `haus.menuBar` (the same distinction the registry makes
two sections up), and the slot is `_contrib.bar.*` either way. Nothing enforces
it, because the whole tree is internal and never rendered.
