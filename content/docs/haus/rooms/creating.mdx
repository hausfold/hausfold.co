...
- **A command on a key.** `haus.keys.leaderExtras` binds the leader plus a key
  to any command; `haus.pounce.items` puts one in the palette.
...
2. **An owner**, in `roomOwners`: which room a namespace belongs to. Two
   namespaces can share one room (`haus.sill` and `haus.menuBar` are both the
...
The rule is: a room may **contribute** a feature to another room, but may never
read the other room's config and may never switch it on.

The seam is an **extension point**. The receiving room declares one, the source
room writes to it, and the receiver renders whatever it finds inside its own
enable gate:

```nix
# in the RECEIVING room's options.nix, here the Bar's
haus._contrib.bar.kettle = contrib.mkExtensionPoint {
  description = "A pill the Kettle room asks the bar to draw.";
  options = { … };
};
```

The middle segment is the receiving **room**, not the module that implements it:
`bar`, not `sill`. Same distinction the registry makes two sections up, and the
reason `haus._contrib.bar.agents` is declared inside `modules/sill/options.nix`
rather than being called `_contrib.sill.*`. Nothing enforces it, because the
whole tree is internal and never rendered.

Two things follow, and both are the point: a bar that is switched off removes
the pill without disabling the kettle, and a kettle that is switched off leaves
the bar's own pills untouched. The alternative (reading
`config.haus.kettle.something` from inside the bar) makes every renamed kettle
option break the bar, and leaves nothing in the tree recording that the
connection exists at all.

`haus._contrib` is internal. The leading underscore is the layer's convention
for wiring between rooms rather than a setting anyone may write, and the doc
renderer prunes those before anything is published.
...
Two rooms have no switch on purpose: `den` (macOS itself) and `hearth` (the
