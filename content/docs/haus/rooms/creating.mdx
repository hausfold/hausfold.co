...
`bar`, not `bar`. Same distinction the registry makes two sections up, and the
reason `haus._contrib.bar.agents` is declared inside `modules/bar/options.nix`
rather than being called `_contrib.bar.*`. Nothing enforces it, because the
