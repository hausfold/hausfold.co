...
and `status` are refused as names. With [the
launcher](/docs/haus/rooms/launcher) on, every scene is a **Scene: recording**
row in ⌘Space with **Leave Scene** beside it; `haus.keys.leaderExtras` with
`command = "$HOME/.local/bin/focus scene recording"` gives it a key.
