...
| `factory` | The merge lease, beside the cup because the two answer the same question: what did I leave switched on, and when does it stop. It draws the time left, an infinity sign for a lease that runs until you revoke it, and its own glyph alone when none stands. Click for 4 hours, 12 hours, until revoked, or Revoke; right-click revokes. Without `factory` on your PATH, which is [`haus.ai.enable`](/docs/haus/rooms/ai), it hides rather than draw a control for a missing tool. [Merging overnight](/docs/haus/night-shift) is the setup it belongs to. |
