...
The way past it is usually to stop writing the domain by hand.
`haus.accessibility.*` reaches every key in it that macOS actually honours —
`increaseContrast`, `differentiateWithoutColor`, `reduceMotion` and
`reduceTransparency` — with a **guarded** write, so without the grant you lose
that setting and nothing else, and the rebuild runs from anywhere. Otherwise:
run it from an app that holds the grant, or set `HAUS_FDA_ANYWAY=1` when you know
better (`HAUS_AGENT_REBUILD=1`, the name this had while the check was
agent-scoped, still works).
