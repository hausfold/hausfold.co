...
anything else is scanned two levels deep. Naming the list replaces it, and repos
scruff already knows are offered whether or not they sit under a root. That last
default entry is the config flake this Mac is built from, which is the repo you
...
(OpenCode) and `pi` (pi), all from nixpkgs, so `haus update` moves them.
`ai.default` must be one of `ai.clients`, and the rebuild says so by name rather
than failing later, inside the pane, after the checkout exists. The client is
recorded **with the lane**, so the option only affects new spawns.
...
`haus.ai.meridian.port` moves the number if something already owns 3456; the
bind address is loopback and is deliberately not an option. It does **not**
follow `haus.ai.enable`, and **haus points nothing at it**: which endpoint a
client uses lives in that client's own file (`~/.pi/agent/models.json`,
OpenCode's config), and those files are yours.

meridian has no credential of its own; it re-uses the OAuth token Claude Code
keeps in your login keychain, so it runs as a **user** agent and the sign-in is
a card in `haus permissions`:
...
alike: one mark and a count per state, always in urgency order. Red is ready for
your turn, sky is working, green is done, and a state with nothing in it draws
nothing at all, so the robot's own colour answers "is anything asking for me"
before you have read a digit.
...
person at the keyboard. Empty, the default, writes no instructions file at all,
so you get the guard and none of the prose; setting it moves a hand-written file
already at that client's path aside as `.backup`, quietly, so look before the
first rebuild after. `HAUS_DESKTOP_OK=1` in a pane's environment switches the
...
and its repos are, `allow` what is ordinary here, `softDeny` what it should stop
and ask about, `hardDeny` what it refuses outright.

Claude Code's own entries stay in front of yours while `ai.autoMode.keepDefaults`
is on; turn that off and yours replace them, refusals included, so read the
result back with `claude auto-mode config` first.
...
`haus.ai.keepAwake` is a switch, not a second copy of the dial. The dials live
with the machinery, in
[`haus.power.lidAwake`](/docs/haus/reference/options#haus-power-lidawake-enable),
and both stops read them: `linger`, how long a hold keeps going past the last
turn; `maxHold`, how long one unbroken hold may last; and `requirePower`, which
guards the **lid** hold only, because unplugging is also how you say stop. The
defaults there are the cautious ones.

One of them is not a dial. `while = "always"` is plain closed-display mode,
agents or no agents, and it is the stop an unattended merge shift wants, because
a foreman waking on a cadence is idle between passes and the agent-shaped hold
drops in the gaps. [Merging pull requests overnight](/docs/haus/night-shift) is
that setup.
