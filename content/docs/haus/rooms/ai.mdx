...
Two words it deliberately isn't: not a *worktree*, because a parked lane has no
checkout on disk and is still a lane you can resume; and not a *session*, which
belongs to your multiplexer and to each client's own transcript. (⌃⌥⇧A makes no
lane at all — it's an agent in the checkout you already have.) Where a lane's
terminal actually lives is the [Development](/docs/haus/rooms/development)
room's call, not holt's: `haus.hearth.lanes.backend` puts it in a zellij pane by
default, or in a window of its own.
...
```nix
haus.ai.clients = [ "claude" "opencode" ];  # what's installed — defaults to [ ]
...
independent, so the switch is yours (or your desktop's) to set — nebelhaus turns
