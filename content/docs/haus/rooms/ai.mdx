...
related:
  - title: "Changing your Mac with an agent"
    ...
    icon: "wand"
  - title: "Development"
    description: "Where ⌃⌘A lives: the rest of the terminal keys, and the ⇧ variants that stay inside a worktree."
...
Press **⌃⌘A** — from any window, not just a terminal — and you get a fresh
...
has no zellij in it to receive one. ⌃⌘A sits in
[windows](/docs/haus/rooms/windows) instead, the one layer that sees every
window — which is why that room has to be on, asserted at build time. ⌘A is
plain select-all again.
</Callout>
...
⌃⌘A keybind, the tab marks, the statusline, the client wiring. Rooms are
...
**clipboard** with a notice saying so, for you to paste. ⌃⌘A never carries a
