...
`haus.terminal.lanes.backend = "zmx"` makes a lane a window, so the bind leaves
the multiplexer for the layer that sees every window: **⌃⌘A**, pressed anywhere,
starts a lane for whatever repo the focused window is looking at. ⌘A and ⌘⇧A
are unbound there and ⌘A goes back to being select-all; ⌃⌥⇧A is unchanged.

That backend also needs `haus.windows.enable`, asserted at build time: the
windows room holds the chord and places each lane.
</Callout>
