...
[factory](https://github.com/hausfold/factory), the ⌘⏎ keybind, the tab marks,
the statusline and the client wiring. `haus.ai.skill`, on by default, adds every
hausfold tool's agent skill to each client, so an agent asked to change this Mac
edits your host file and runs `haus rebuild` rather than guessing at dotfiles.
...
| Key | Does |
|---|---|
| `↵` | Spawn **in the background**: checkout, branch, agent and window all happen, focus never moves, and a notification says the lane is up |
| `⇧↵` | Newline |
| `⌘↵` | Screenshot first, then spawn; cancel the capture and your text is intact |
| `⌃↵` | Spawn **and follow it**: the window opens now and you go with it |
| `⌥↵` | Drafts; the box is saved on every dismissal |
| `⇥` | Cycle the **client** chip, which agent this one lane opens in (`⇧⇥` goes back) |

The chip appears once more than one of `claude`, `codex`, `opencode` and `pi` is
installed, and offers what is on `PATH`, not what `haus.ai.clients` asked for. It
opens on [`haus.ai.default`](/docs/haus/reference/options#ai), then on your last
spawn. Repos come as a grid of cards, most recently touched first: `↑↓` moves a
row, `←→` a card.

A background lane gets its window at once, tiled on `T/<repo>` with the agent
already working, born off-screen and walked onto its page without ever being
activated, so `⌃⇥` finds it. With [trill](/docs/trill) the banner is the door:
click the card and you land on that window. Without trill it is a plain macOS
banner with nothing to click, and so is a repo or lane name carrying a character
...
`SCRUFF_NAMER_FALLBACK` for an adapter that cannot reach its model to print
instead: honour it, or an offline spawn is named worse than it would have been.
`claude` is scruff's one built-in and needs no file, but at 8 to 12 seconds it is
slower than the palette waits, so Spawn Agent keeps its slug and only hand-run
`scruff spawn` asks it.
...
in silence, naming the list replaces it, and repos scruff already knows are
offered whether or not they sit under a root. That last default entry is the
config flake this Mac is built from, which is the repo you most often want an
agent in.
...
reason; and `pi`, because `--` (end-of-options) reached it in 0.84.3 and scruff
puts a `--` before a lane's first-turn prompt, so on an older pi a task starting
with a dash is read as a flag and the pane dies before the agent draws. Both step
aside once nixpkgs passes them.
...
without asking who is calling is safe on loopback and is an open account on any
...
The dot shows the most urgent state in the tab, so peach wins and the tab that
needs you never hides behind one that is merely busy. The shape carries the
state, not the colour, and there is no count.
...
<Callout type="info" title="What haus writes into pi's files">
pi has no hook file, so its wiring is two extensions in
`~/.pi/agent/extensions/`. `haus-agent-state.ts` reports the four marks and hands
`scruff hook notify` the same payload Claude Code does, so a pi lane's **ask**
and **done** banners are keyed, clickable and cleared the same way;
`haus-desktop-guard.ts` is pi's half of
[the screen guard](#the-screen-belongs-to-you). Both are gated on pi running as a
real pane, since a `pi -p` in a script inherits the session it was run from.

pi has no permission modes, so "blocked on you" is a tool whose execution *is* a
question, `ask_user_question`, which puts the Allow/Deny on the banner. Two pi
events have no Claude counterpart and carry their own `rules.json` source, so you
can silence one without the other: a failed compaction (`haus.ai.pi.compact`) and
the provider refusing the session with a 401/402/403 (`haus.ai.pi.auth`, once).
The second matters with meridian, which runs on Claude Code's keychain token:
when that goes stale every pi lane stops and says so nowhere.

In `~/.pi/agent/settings.json`, `tuiMode = "fullscreen"` and `quietStartup` are
re-asserted every rebuild; `hideThinkingBlock` and `modelThinkingLevels` are
seeded once, because pi's own `/settings` changes them mid-session.
`haus.ai.pi.packages` is merged into `packages` beside whatever `pi install` put
there, so removing an entry does **not** uninstall it: run `pi remove <source>`
once. `models.json` and `trust.json` stay yours.
</Callout>
...
person at the keyboard. Leave it empty, the default, and haus writes no
instructions file at all, so you get the guard and none of the prose. Set it and
a hand-written file already at that client's path is moved aside as `.backup`,
quietly, so look before the first rebuild after.
`HAUS_DESKTOP_OK=1` in a pane's environment switches the guard off there, for a
long unattended run where forty prompts is the problem. Claude Code and pi are
the two that get the guard; the etiquette goes to every client.
...
lists of prose, one fact or rule per string:
...
| `allow` | what is ordinary here, as exceptions to the classifier's own refusals |
...
`haus.ai.keepAwake` is a switch, not a second copy of the dial. The defaults are
the cautious ones.

| | |
|---|---|
| `linger` (5 min) | keeps holding past the last turn, so the gap between two turns cannot sleep the Mac |
| `maxHold` (8 hours) | caps one unbroken hold, so a client that dies without reporting costs you one window, not a Mac that never sleeps again. A genuinely new agent lifts the cap early |
| `requirePower` (on) | the **lid** hold only: unplugging is also how you say stop. The `idle` hold ignores it |
| `while = "always"` | plain closed-display mode, agents or no agents |

`always` is the stop an unattended merge shift wants, because a foreman waking on
