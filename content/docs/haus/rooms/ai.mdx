...
than replacing it, leaves the four agent-state hooks and any hook of your own
alone, and re-asserts on every rebuild, because Claude Code rewrites its own
...
The same rebuild prepends a short **screen etiquette** section to the
instructions file every client reads: prefer looking to touching, never
foreground an app just to see it, hand feel-tests back to the person at the
keyboard. The etiquette is what keeps the hook from firing; the hook is what
catches the times it doesn't.
...
`BENCH_AGENT_SWITCH=1`: a reminder, not a jail. Only Claude Code gets the hook,
since it is the client with a `PreToolUse` event to hang it on; the etiquette
text goes to all of them.
</Callout>
