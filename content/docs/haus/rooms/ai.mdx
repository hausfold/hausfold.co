...
leaves 25 bytes of lane name, one called `nix` leaves 33. A name scruff or a
namer chose is trimmed to fit at a word boundary, and a name you typed is
refused with both numbers rather than quietly shortened. A refusal from the
palette is not on screen anywhere: Spawn Agent toasts that it could not create
the worktree, and the numbers are in `~/.cache/haus/spawn-agent.log`.
