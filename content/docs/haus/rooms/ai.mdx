...
about it: it is simply empty, so it gets the muted `●`, not `⏏`. The same
distinction reaches the paw pill — such a lane draws no PR row at all, where it
used to announce itself as `merged` from the second it was spawned.
