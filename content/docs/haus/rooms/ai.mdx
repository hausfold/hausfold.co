...
`haus.ai.keepAwake` is a switch, not a second copy of the dial. How long a hold
lingers past the last turn, how long one may last, and whether unplugging ends it
live with the machinery, in
[`haus.power.lidAwake`](/docs/haus/reference/options#haus-power-lidawake-enable),
and the defaults there are the cautious ones.
