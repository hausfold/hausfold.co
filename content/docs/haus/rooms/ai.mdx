...
A lane is a window, not a pane, so the bind can't live in the multiplexer:
zellij's only way to run a command is to open a pane, and a lane's own window
has no zellij in it to receive one. ⌘⏎ is held by
...
is sitting on a permission prompt? For lanes that's the [bar](/docs/haus/rooms/bar)'s
`agents` pill, since a lane has a window rather than a tab. For a resident
⌃⌥⇧A agent the zellij tab bar answers it, with a coloured dot in front of the
name of any tab holding one:
