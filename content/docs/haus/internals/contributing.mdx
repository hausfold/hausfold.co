...
none of it — [Keeping it current](/docs/haus/guides/keeping-it-current) is that
page.
...
| **`holt`** | manage [agent worktrees](/docs/haus/guides/coding-agents) in any git repo | haus, with `developer.agents.enable` on |
| **`zscratch`** | feel-test a zellij edit with no rebuild | haus, same switch |
...
its own worktree. That mechanism is [holt](/docs/haus/guides/coding-agents) and
...
<Callout title="Releases ride tags, and the version is the date">
Four repos are releasable — the launcher, the shelf, haus and holt — and the
number is never typed by hand. `bench release <repo>` stamps today's date
...
- [AeroSpace](https://github.com/nikitabobko/AeroSpace) — tiling; [SketchyBar](https://github.com/FelixKratz/SketchyBar) — the bar
