...
<Callout title="This used to need its own tool">
haus ran on [zellij](https://zellij.dev) until 2026-08-19, and iterating on it
needed a fourth CLI (`zscratch`) plus an activation hack, for two reasons worth
knowing only so nobody rebuilds them. A running zellij server cached plugin wasm
in memory for its whole lifetime, so a plugin edit needed a *fresh* server,
which meant losing every open tab, which is what `zscratch`'s throwaway session
avoided. And zellij decided "the config changed" by file mtime, while every
`/nix/store` file is stamped epoch 1, so a home-manager symlink made each
rebuild look *older* than what the server had already read and nothing reloaded
, which is why that one file had to be installed by an activation script instead
of linked. Ghostty has no plugins and watches a store symlink happily.
</Callout>
...
- the terminal stack: [Ghostty](https://ghostty.org), [zmx](https://github.com/neurosnap/zmx), [yazi](https://yazi-rs.github.io), [helix](https://helix-editor.com), [starship](https://starship.rs)

haus, nebelung, pounce, perch and trill are MIT; **holt** is Apache-2.0. Read
