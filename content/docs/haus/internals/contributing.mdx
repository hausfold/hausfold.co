...
<Callout title="No extra tooling for this">
Ghostty has no plugins and watches a store symlink happily, so there is nothing
to scratch-session, nothing to reinstall by activation script and no fourth CLI
in the loop. Edit, rebuild, keep working.
</Callout>
