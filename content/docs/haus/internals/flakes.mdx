...
(The input is named `nebelhaus` because it names the desktop you installed rather
than the layer under it. It's a local handle; nothing outside your flake reads
it.)

Your choices live in `hosts/<hostname>/default.nix` — [the one file you
edit](/docs/haus/guides/making-it-yours). haus stays upstream, and you never
edit it.

## The builder

`mkNebelhaus` is the entry point. Given a user, a hostname and a host file it
...
nebelhaus.mkNebelhaus {
  ...
  extraModules = [ ];          # more nix-darwin modules, or a rice
}
```

Only `username` and `hostname` are required. `extraModules` is the seam a
[shared rice](/docs/haus/guides/sharing-a-rice) arrives through, and the one the
installer uses to import the preset you picked. Eight of haus's modules — `den`,
...
declares no options of its own and takes the username from `mkNebelhaus`, so on
its own it doesn't evaluate.

## The inputs

The haus flake pulls together nine:
...
| `holt` | the worktree substrate, put on your `PATH` when `developer.agents.enable` is on |
...
You never walk it by hand. [`haus update`](/docs/haus/guides/keeping-it-current#pulling-a-newer-haus)
