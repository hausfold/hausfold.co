...
| `trill` | the notification compositor, packaged the same way as perch. The only input a room installs and no room needs: `haus.trill.enable` places the bundle, and `haus-notify` finds it at runtime or falls back to Apple's banner |
