...
same per-client directory. Four of them come from the tools, on any machine:
`scruff` (the agent-worktree lifecycle: what's open, resume a parked lane, set
work aside without `git stash`), `handoff` (turning a piece of work into a brief
a cold session can act on, ending on the clipboard or in a new lane), `factory`
([merging the pull requests](/docs/haus/night-shift) a filter you typed can
vouch for, while nobody is watching), and `nebelung` (this machine's exact
palette, rendered from the revision you're pinned to rather than remembered).
