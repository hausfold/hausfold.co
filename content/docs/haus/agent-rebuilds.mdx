...
on a machine running a client haus didn't install. jcode is the one exception
there: creating `~/.jcode/` is itself an action — it's what stops jcode copying
your Claude and Codex skills into its own directory on first run — so it gets a
copy only when you name it. One body, one copy per client:
...
(seven of them, which since August 2026 is **every key nix-darwin types there**,
so the raw form no longer reaches anything the safe one doesn't) with a
...
#   ~/.claude/skills/haus · ~/.codex/skills/haus · ~/.config/opencode/skills/haus · ~/.jcode/skills/haus
...
`~/.config/opencode/AGENTS.md`, `~/.jcode/prompt-overlay.md`), under a short
generated preamble.
...
aside to `<file>.backup`. And `haus.ai.skill = false;` stops the skill being written
...
[`/llms.txt`](/llms.txt) is a table of contents with one-line descriptions, for
something that fetches selectively; [`/llms-full.txt`](/llms-full.txt) is every
page inlined, for something that would rather read the lot. Any page is also
available as its own Markdown, from the button above its title.
