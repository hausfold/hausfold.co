...
same per-client directory. Four of them come from the tools, on any machine: `scruff`
...
session can act on, ending on the clipboard or in a new lane), `factory`
([merging the pull requests](/docs/haus/night-shift) a filter you typed can
vouch for, while nobody is watching), and `nebelung`
