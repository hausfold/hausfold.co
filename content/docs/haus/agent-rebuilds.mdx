...
what's open, resume a parked lane, set work aside without `git stash`),
`handoff` (turning a piece of work into a brief a cold session can act on,
ending on the clipboard or in a new lane), and factory's `factory` and
`nightshift` (merging the pull requests a filter you typed can vouch for, and
the loop that does it while nobody is watching). A tool whose room is *optional*
joins them only when you switch that room on: `trill` (sending a notification through
