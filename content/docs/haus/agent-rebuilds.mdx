...
same per-client directory — today `holt` (the agent-worktree lifecycle: what's
open, resume a parked lane, set work aside without `git stash`) and `handoff`
(turning a piece of work into a brief a cold session can act on, ending on the
clipboard or in a new lane). Each tool owns and versions its own skill; haus
only decides that they are installed, so a lock bump can bring a better one
