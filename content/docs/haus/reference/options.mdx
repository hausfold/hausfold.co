...
every Zen profile. A rebuild recolours them; nothing to re-import, no
state to carry to the next machine.

A slug is the style's own name in nebelung's bundle — `github`,
`youtube`, `reddit`, `wikipedia`. Naming one that doesn't exist fails
...
~320 KB, all 134 would be several megabytes of CSS on every page load
to theme sites you never open. This is for the handful you actually
read.
