...
can own more than one: the Bar room is `haus.sill` (its own bar) *and*
...
`string` · default `the command for haus.hearth.editorName — hx for helix`
...
The stock menu bar: what the clock shows, and which Control Center glyphs sit beside it. (The hacker bar itself is `sill`.)
...
the rest of Sill. Disable this to draw them in `haus.fonts.sans.name`
...
#### `haus.sill.github.sources.*.command`
...
#### `haus.sill.github.sources.*.limit`
...
#### `haus.sill.github.sources.*.search`
...
#### `haus.sill.github.sources.*.severity`
...
#### `haus.sill.github.sources.*.title`
