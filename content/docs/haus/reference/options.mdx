...
The Appearance room's own profile — one named answer to a whole-machine question, where the groups below are the individual dials. `largePrint` sets the interface scale, the high-contrast palette, macOS's own contrast lift and the screen's scaled resolution together, each as a default you can still pin by hand.
...
#### `haus.terminal.rightClickFullscreen`
...
`one of "latest", "claude", "codex", "opencode", "jcode"` · default `"latest"`
...
Pounce — the command palette, its daemon, its commands, and every Pounce setting haus exposes.
...
Perch — the file shelf that grows out of the notch to catch what you drag at it.
...
`list of (one of "claude", "codex", "opencode", "jcode")` · default `[ ]`
...
`one of "claude", "codex", "opencode", "jcode"` · default `"claude"`
