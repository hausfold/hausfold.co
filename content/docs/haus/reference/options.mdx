...
The Focus (Do-Not-Disturb) pill. Needs `haus.focus.enable`; setting this moves the pill but does not enable the Focus room by itself.
...
```nix
"security find-generic-password -s focus-slack -w"
```
