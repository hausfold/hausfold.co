...
#### `haus.apps.packs.writing.enable`
...
`one of "none", "minimal", "orbits", "constellation", "flow", "bold"` · default `"none"`
...
`positive integer, meaning >0` · default `13`
...
`positive integer, meaning >0` · default `fonts.mono.baseSize, scaled by haus.ui.scale and rounded`
...
#### `haus.fonts.sans.name`

`string` · default `".AppleSystemUIFont"`
...
#### `haus.accessibility.closeViewScrollWheelToggle`
...
#### `haus.accessibility.closeViewZoomFollowsFocus`
...
#### `haus.accessibility.mouseDriverCursorSize`

`null or integer or floating point number between 1.0 and 4.0 (both inclusive)` · default `null`
...
#### `haus.accessibility.reduceMotion`
...
#### `haus.accessibility.reduceTransparency`
...
#### `haus.terminal.editor`

`string` · default `the command for haus.terminal.editorName — hx for helix`

**Host-only.** A shared desktop may not set it; only your host file can. It is a shell command this machine runs. A desktop is a file you can read to know what it does, and a leaf that runs a command is exactly what would stop being true of.
...
`one of "helix", "nano", "neovim", "vim"` · default `"helix"`
...
`list of value "node" (singular enum)` · default `[ ]`
...
#### `haus.windows.accordionPadding`

`unsigned integer, meaning >=0` · default `80`
...
#### `haus.windows.defaultOrientation`
...
#### `haus.windows.desktop.clickToReveal`
...
#### `haus.windows.desktop.hideIcons`
...
#### `haus.windows.desktop.hideWidgets`
...
#### `haus.windows.mouseFullscreen`

`one of "right", "left", "none"` · default `"none"`
...
#### `haus.windows.nativeTiling.edgeDrag`
...
#### `haus.windows.nativeTiling.margins`
...
#### `haus.windows.nativeTiling.optionAccelerator`
...
#### `haus.windows.nativeTiling.topEdgeFullscreen`
...
#### `haus.windows.numberedWorkspaces`

`integer between 0 and 10 (both inclusive)` · default `4`
...
#### `haus.windows.stageManager.autoHideStrip`
...
#### `haus.windows.stageManager.enable`
...
#### `haus.windows.stageManager.groupWindows`
...
#### `haus.windows.stageManager.hideDesktopIcons`
...
#### `haus.windows.stageManager.hideWidgets`
...
#### `haus.bar.bottom.items.calendar`
...
#### `haus.bar.calendar.me`

`list of string` · default `[ ]`

**Host-only.** A shared desktop may not set it; only your host file can. It names you rather than a machine — your commit identity, the addresses that are yours. A desktop that set it would put its author's name on your work.
...
#### `haus.bar.elgato.host`

`string` · default `""`

**Host-only.** A shared desktop may not set it; only your host file can. It names a device on your own network by host or IP — a fact about your desk. Left empty, the pill discovers the device itself, which is what a shared desktop should leave it doing.
...
`integer between 60 and 3600 (both inclusive)` · default `300`
...
#### `haus.bar.github.sources.*.command`
...
#### `haus.bar.github.sources.*.limit`

`integer between 1 and 100 (both inclusive)` · default `8`
...
#### `haus.bar.github.sources.*.search`
...
```nix
"org:hausfold is:pr is:open"
```

<small>Declared in [`modules/bar/options.nix`](https://github.com/hausfold/haus/blob/main/modules/bar/options.nix).</small>

#### `haus.bar.github.sources.*.severity`

`null or one of "info", "warn", "bad"` · default ``bad` for a `ci` source, `info` for the others`
...
```nix
"/etc/haus/backup-status.sh"
```

<small>Declared in [`modules/bar/options.nix`](https://github.com/hausfold/haus/blob/main/modules/bar/options.nix).</small>

#### `haus.bar.widgets.<name>.enable`
...
#### `haus.bar.widgets.<name>.interval`

`null or (positive integer, meaning >0)` · default `null`
...
#### `haus.bar.widgets.<name>.permissions`
...
#### `haus.bar.widgets.<name>.placement`

`null or one of "menu-bar", "bottom-left", "bottom-center", "bottom-right", "left", "center", "right"` · default `null`
...
#### `haus.launcher.autoQuit.exclude`
...
**Desktop-safe per key** (`launcher-items`). Keys are plain item ids, since each one names a single row in the palette.
...
#### `haus.launcher.items.<name>.alias`
...
#### `haus.launcher.items.<name>.caption`
...
#### `haus.launcher.items.<name>.hotkey`
...
#### `haus.launcher.items.<name>.listed`
...
`integer or floating point number between 0.8 and 2.0 (both inclusive)` · default `haus.ui.scale, held inside the palette's 0.8-2.0`
...
One quiet switch: Do Not Disturb, an optional status somewhere else, and your own hooks on both edges — plus the named states (`scenes`) around it, of which quiet is the built-in one.
...
#### `haus.focus.scenes.<name>.apps.closeOnExit`
...
#### `haus.focus.scenes.<name>.apps.open`
...
#### `haus.focus.scenes.<name>.audio.input`
...
#### `haus.focus.scenes.<name>.description`
...
```nix
"camera on, nothing interrupts"
```

<small>Declared in [`modules/focus/options.nix`](https://github.com/hausfold/haus/blob/main/modules/focus/options.nix).</small>

#### `haus.focus.scenes.<name>.dnd`
...
#### `haus.focus.scenes.<name>.preventSleep`
...
#### `haus.focus.scenes.<name>.restorePreviousState`
...
**Host-only.** A shared desktop may not set it; only your host file can. It points at a secret, or at the store this machine keeps its secrets in — a credential belonging to one person on one Mac.
...
```nix
"If found, please call +1 555 0100."
```

<small>Declared in [`modules/core/options.nix`](https://github.com/hausfold/haus/blob/main/modules/core/options.nix).</small>

#### `haus.lock.login.showNameField`
...
`one of "caps", "alt-space", "none"` · default `"none"`
...
`one of "cmd-space", "alt-space", "ctrl-space", "none"` · default `"none"`
...
`one of "alt", "ctrl-alt", "cmd-alt", "none"` · default `"none"`
