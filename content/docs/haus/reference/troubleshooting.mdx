...
Pounce is a launchd **user agent**. If it isn't answering, bounce it — this is
the clean recovery for *any* wedged agent, and the rest of this page reuses it:
...
session was ready parked itself instead of starting. Bounce it:
...
land? The easy fix is **Reload SketchyBar** from the ⌘Space palette, or the same
row in the logo pill's dropdown. By hand:
...
which has no CLI to set it. So it's a one-time manual step:
...
<Step>
Scroll to **Allow in the Background** and find the entries named **`sh`**,
subtitled *Item from unidentified developer*.
</Step>

<Step>
Toggle them **on** and reboot. Already on but still blocked? Flip off, then on,
to force a database write.
</Step>
