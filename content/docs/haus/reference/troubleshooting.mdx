...
Set `haus.pounce.signingIdentity` so that grant survives rebuilds — see
[granting Accessibility](/docs/haus/guides/pounce#grant-accessibility). If
...
detects a Claude Code session that lacks it. Better still, sidestep the domain:
`haus.accessibility.increaseContrast` and
`haus.accessibility.differentiateWithoutColor` reach the two keys in it that are
worth having, with a guarded write. [Changing your Mac with an
