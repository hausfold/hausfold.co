...
```sh
haus doctor      # Nix, the Xcode tools, the background jobs that want
                 # attention, cask drift, the launcher's Accessibility grant,
                 # the theme ports and your secrets, in one pass
haus services    # the whole roster: every background job, what it's for,
                 # whether it's running or idle by design, and where it logs
```

`haus doctor` catches a job whose last run failed, not only one that is dead
right now, so a weekly cleanup of the Nix store that has quietly been exiting
non-zero every Sunday gets a line.

Reading a job's error log is usually faster than restarting blind, and a job
that dies again ten seconds after a kick is telling you something that only
shows up in it. `haus services` prints each log's real path rather than guessing
at one: the tiler, the bar and the launcher write `<name>.out.log` and
`<name>.err.log` in `/tmp`, but a job that came with another room logs under
`~/Library/Logs` or `/var/log`. [What this Mac runs in the
background](/docs/haus/reference/haus#what-this-mac-runs-in-the-background) is
the roster and what each job is for.
...
The daemon runs the CI-built release app, signed with the hausfold Developer
ID, so that grant survives rebuilds on its own; see
...
Still nothing? Run `haus report`. It opens haus's bug form with the output of
`haus doctor`, your macOS build, your Mac model and the desktop you're on
already in the diagnostics field, so all that's left to write is what happened.
On a full machine that report is too long to ride in a link: the form opens with
the field empty and the block goes to your terminal (or your clipboard, if you
came from the palette) to paste in. Either way it prints the block first, and
nothing is filed until you press Submit. `haus report --print` shows the same
block and the same link without opening a browser.
