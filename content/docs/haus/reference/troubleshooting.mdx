...
rebuild runs. Everything your config declares is stamped `trusted: true` in the
Brewfile activation hands to `brew bundle`, so a rebuild never meets this.
Seeing it means the cask came from somewhere haus doesn't know about: a tap you
added by hand, or a `brew bundle` of your own. Declare it in your config and
`haus rebuild`.
