...
right now, so a weekly Nix store cleanup that has quietly been exiting non-zero
every Sunday gets a line. Reading that job's log beats restarting blind, and
...
ID, so that grant survives rebuilds on its own; see [Granting
...
instead of hitting the daemon's fast path. Registration reports success and the
palette still opens, which is what makes it hard to spot; [`pounce
doctor`](/docs/pounce/install#check-it-landed) names the culprit.
...
`haus permissions` walks you through granting it, `haus plan` says whether the
next rebuild needs it, and `haus.accessibility.*` sidesteps the domain for good.
[The one rebuild it
...
Still nothing? `haus report` opens haus's bug form with `haus doctor`'s output,
your macOS build, your Mac model and the desktop you're on already in the
diagnostics field, so all that's left to write is what happened. On a full
machine that report is too long to ride in a link: the form opens empty and the
block goes to your terminal, or your clipboard if you came from the palette, to
paste in. Either way it prints the block first, and nothing is filed until you
press Submit. `haus report --print` shows the same block without opening a
browser.
