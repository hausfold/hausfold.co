...
immediately. It stages the file too, which matters: neither `haus show` nor
`checkDesktop` catches a missing `git add`, because both read what they are
handed and pass on the very file the rebuild cannot see.
</Callout>
...
Not how to override you: a host beats a desktop with a plain assignment, every
time, without either of you doing anything.
...
leave one Mac; until haus can record which input claimed a name, a machine that
installs yours warns once per rebuild that nothing says where it came from.
