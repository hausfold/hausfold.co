...
`haus.terminal.ghDash.enable = true` sits beside it.
...
own](/docs/haus/rooms/apps); [the bar](/docs/haus/rooms/bar) lists its pills,
[Development](/docs/haus/rooms/development) the editor.
...
Caps Lock stays Caps Lock. Only `cmd-space` displaces Spotlight, so the launcher
has the same escape:
[`haus.keys.palette`](/docs/haus/rooms/keys#not-fond-of-these-keys) also takes
`alt-space`, `ctrl-space` or `"none"`.
...
Two more are nix-darwin's own, and they lie.
**`NSGlobalDomain.AppleInterfaceStyle`** is inert on macOS 26 and mirrors the
appearance back at you, so a plist read-back calls an inert write applied: use
`haus.theme.systemAppearance`, which `haus diff` checks it against.
**`power.sleep.computer`** and its neighbours run `systemsetup`, which cannot say
*which* power source you mean, and one of them wrote the **charger** profile
while the Mac was on **battery**: use `haus.power.*`, which says `battery` and
`charger` separately.
</Callout>
...
Add a [gitleaks](https://github.com/gitleaks/gitleaks) pre-commit hook so a stray
key never gets to *be* a commit, and scan the history you have **before** you
flip it public. Deleting a line later doesn't unpublish the key, so rotate
anything it finds.
