...
| `/mcp.json`, `/.well-known/mcp.json` | the two MCP manifests, in two different schemas | 🚨 **Not aliases, and `/.well-known/mcp.json` is not `/.well-known/mcp`.** The root one is the agent-plugins.org `mcpServers` map, which is a client config file; the well-known one is the flat shape a scanner probing that path parses (top-level `url` + `transport`, then `servers`, then `MCP_TOOLS`). Both are generated from `MCP_SERVER_INFO`/`MCP_TOOLS`, so they and `initialize` cannot describe three servers. `/.well-known/mcp` one suffix away is the **transport**, and GET there is a 405 that Streamable HTTP requires: a test in `test/agent-surface.test.js` holds that line, because collapsing the two is the obvious tidy-up and it would break the endpoint |
