...
| `/developers` | `src/app/developers/page.tsx` | the machine-facing half, written down for agents and CI. Every fact on it is read off `worker.js` and pinned by `test/openapi.test.js`; a link here to a route the Worker does not answer is a claim the products don't back |
