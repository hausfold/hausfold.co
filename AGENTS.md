...
| the **OpenAI app listing** — the starter prompts, the eight test cases, the nine tool-annotation justifications, the demo video | `scripts/submit-openai-app.sh`, a wizard that paces the whole submission through the clipboard one field at a time. Its arrays are written against `MCP_TOOLS` and **nothing checks them**: a fourth tool means three more justifications, a starter that reaches it and a case that proves it. `submit-openai-app.sh check` probes the live server, not the file |
