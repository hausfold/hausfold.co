...
| `.well-known/ard.json` | Agentic Resource Discovery catalog listing the two MCP servers (full and docs-only), the OpenAPI spec, the Agent Plugin manifest and the `haus-install` skill, for AI clients that probe a domain before reading anything else. ⚠️ **It does not list either on-domain MCP manifest** (`/mcp.json`, `/.well-known/mcp.json`), and the entry that says "manifest" is `plugin.json` on GitHub, not one of those |
