...
| Worktree hooks | `~/.claude/settings.json` (yours, not the repo's) → `holt hook create` / `holt hook remove` | Declared by haus and re-asserted every rebuild, so no repo in the family hand-edits that file. |
